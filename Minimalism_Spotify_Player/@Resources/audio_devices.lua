...
function RefreshDevices()
    ...
    -- ХИТРОСТЬ: Используем "подстановочные знаки", чтобы PowerShell не подавился скобками
    -- Мы заменяем путь на такой, который PowerShell поймет без кавычек
    local psPath = TMP_FILE:gsub('%(', '`('):gsub('%)', '`)')

   -- Используем -Encoding Default (это системная кодировка Windows, обычно Windows-1251 для кириллицы)
    -- И добавили [Console]::OutputEncoding, чтобы PowerShell выдавал текст правильно
    local psCommand = [[
